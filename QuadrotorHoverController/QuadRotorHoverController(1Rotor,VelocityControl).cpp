...
inline void controlMatrices(const Input& uGoal, const Matrix<X_DIM, X_DIM>& Qx, const Matrix<X_DIM,1>& xstar, const Matrix<V_DIM,V_DIM>& Qv, const Matrix<V_DIM,X_DIM>& V, const Matrix<U_DIM, U_DIM>& R, Matrix<U_DIM,X_DIM>& L, Matrix<U_DIM,V_DIM>& E, Matrix<U_DIM,1>& l) {
...
int _tmain(int argc, _TCHAR* argv[])
{
  ...
  xInit[0] = 0; xInit[1] = 0; xInit[2] = 3; // pos
  ...
  xGoal[0] = 0; xGoal[1] = 0; xGoal[2] = 2;
  xGoal[3] = 10;  xGoal[4] = 5;  xGoal[5] = 0;
  xGoal[6] = 0;  xGoal[7] = 0;  xGoal[8] = 0;
  xGoal[9] = 0; xGoal[10] = 0; xGoal[11] = 0;
  ...
  Matrix<X_DIM, X_DIM> Qx = zeros<X_DIM,X_DIM>();
  // Position & Velocity = 0;
	Qx(6,6) = Qx(7,7) = 1; Qx(8,8) = 20;
	Qx(9,9) = Qx(10,10) = Qx(11,11) = 1;

	Matrix<V_DIM,V_DIM> Qv = zeros<V_DIM,V_DIM>();
	Qv(0,0) = Qv(1,1) = Qv(2,2) = 10;
  ...
  clock_t beginTime = clock();		// begin timing the operation

  State x = xInit;				// initializes the state at the initial state
  Rotation Rot = RotInit;

  State xTrue = sampleGaussian(xInit, P);	// adds some gaussian noise to the state in order to simulate state estimation error in sensors
  Rotation RotTrue = RotInit;			
  RotTrue = RotTrue*exp(skewSymmetric(xTrue.subMatrix<3,1>(6,0)));
  xTrue[6] = 0; xTrue[7] = 0; xTrue[8] = 0;

  Matrix<3,X_DIM> V = zeros<3,X_DIM>();
  V(0,3)=1; V(1,4) = 1; V(2,5) = 1;
  Matrix<U_DIM,X_DIM> L; Matrix<U_DIM,X_DIM> E; Matrix<U_DIM,1> l;
  controlMatrices(uGoal,Qx,xGoal,Qv, V, R, L, E, l);		// finds the control matrices L E and ell for LQR

  Matrix<X_DIM,X_DIM> S = Qx;	// sets the value for S in	LQR

  for (size_t t = 0; t < 320; ++t) {		// why 320?
	// High level controller: in: desired position, current state (estimate), out: desired velocity
    // Obtain xGoal from collision avoidance: in: desired velociy, out: valid goal velocity

    // Low level controller: in: goal velocity: out: control input u (lowest level control inputs, e.g. motor voltages)
	Input u = riccatiControllerSteady(x, Rot, xGoal, RotGoal, uGoal, L, E, l);   // loops through a given x to achieve xgoal given some L E and ell matrices
