...
#define NUM_QUADS 5			// Number of quadrotors
...
public:
  ...
  void setupQuadrotors(const State& xinit, const Rotation& rotinit, const Matrix<X_DIM,X_DIM> Pinit, const State& xfinal, const Velocity& vfinal, const Input& uHover, const Rotation& rotfinal) {
    x = xinit;
    Rot = rotinit;
    xGoal = xfinal;
    vGoal = vfinal;
  ...
  void findMatrices(Matrix<X_DIM,X_DIM>& A, Matrix<X_DIM,U_DIM>& B, Matrix<X_DIM,1>& c) {
    controlMatrices(uGoal, xGoal, A, B, c, L, E, l);
  }
...
inline void controlMatrices(const Input& uGoal, const Matrix<X_DIM,1>& xstar, Matrix<X_DIM,X_DIM>& A, Matrix<X_DIM,U_DIM>& B, Matrix<X_DIM,1>& c, Matrix<U_DIM,X_DIM>& L, Matrix<U_DIM,V_DIM>& E, Matrix<U_DIM,1>& l) {
...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	Velocity VGoal0, VGoal1, VGoal2, VGoal3, VGoal4, VGoal5, VGoal6, VGoal7;
	Position PGoal0, PGoal1, PGoal2, PGoal3;
	xInit[12] = xInit[13] = xInit[14] = xInit[15] = nominalInput;

	// Quadrotor 0
	xInit[0] = -4.0; xInit[1] = 0.0; xInit[2] = 2.0;
	xInit[3] = 1.0; xInit[4] = 0.0; xInit[5] = 0;
	VGoal0[0]= 2.0; VGoal0[1] = 0.0; VGoal0[2] = 0.0;
	qlist[0]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal0, uGoal, RotGoal);
	// Quadrotor 1
	xInit[0] = 4.0; xInit[1] = 0.0; xInit[2] = 2.0;
	xInit[3] = -1.0; xInit[4] = 0.0; xInit[5] = 0;
	VGoal1[0] = -1.0; VGoal1[1] = 0.0; VGoal1[2] = 0.0;
	qlist[1]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal1, uGoal, RotGoal);
	// Quadrotor 2
	xInit[0] = 0.0; xInit[1] = 4.0; xInit[2] = 2.0;
	xInit[3] = -0.0; xInit[4] = -1.00; xInit[5] = 0.0;
	VGoal2[0] = -0.0; VGoal2[1] = -1.0; VGoal2[2] = 0.0;
	qlist[2]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal2, uGoal, RotGoal);
	// Quadrotor 3
	xInit[0] = 0.0; xInit[1] = -4.0; xInit[2] = 2;
	...
	xInit[3] = -arm/4.0; xInit[4] = -arm/4.0; xInit[5] = 0;
	VGoal4[0] = -arm/4.0; VGoal4[1] = -arm/4.0; VGoal4[2] = 0;
	qlist[4]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal4, uGoal, RotGoal);
	/*// Quadrotor 5
	xInit[0] = -arm; xInit[1] = arm; xInit[2] = 2;
	xInit[3] = arm/4.0; xInit[4] = -arm/4.0; xInit[5] = 0;
	VGoal5[0] = arm/4.0; VGoal5[1] = -arm/4.0; VGoal5[2] = 0;
	qlist[5]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal5, uGoal, RotGoal);
	// Quadrotor 6
	xInit[0] = -arm; xInit[1] = -arm; xInit[2] = 2;
	xInit[3] = arm/4.0; xInit[4] = arm/4.0; xInit[5] = 0;
	VGoal6[0] = arm/4.0; VGoal6[1] = arm/4.0; VGoal6[2] = 0;
	qlist[6]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal6, uGoal, RotGoal);
	//Quadrotor 7
	xInit[0] = arm; xInit[1] = -arm; xInit[2] = 2;
	xInit[3] = -arm/4.0; xInit[4] = arm/4.0; xInit[5] = 0;
	VGoal7[0] = -arm/4.0; VGoal7[1] = arm/4.0; VGoal7[2] = 0;
	qlist[7]->setupQuadrotors(xInit, RotInit, Pinit, xGoal, VGoal7, uGoal, RotGoal);*/
	...
	for(size_t t = 0; t < TIME_STEPS; ++t) {
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[0]->L, qlist[0]->E, Ft, Gt);
			createObstacle(qlist[0]->reachablePoints,points,qlist[0]->Transform,qlist[0]->Translate,t,Ft,Gt,C,qlist[0]->x,qlist[4]->x);
		}
		pointInHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[0]->x, qlist[4]->x, qlist[0]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		/*//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[0]->L, qlist[0]->E, Ft, Gt);
			createObstacle(qlist[0]->reachablePoints,points,qlist[0]->Transform,qlist[0]->Translate,t,Ft,Gt,C,qlist[0]->x,qlist[5]->x);
		}
		pointInHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[0]->x, qlist[5]->x, qlist[0]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		// Rotor 0 Half Planes
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[0]->L, qlist[0]->E, Ft, Gt);
			createObstacle(qlist[0]->reachablePoints,points,qlist[0]->Transform,qlist[0]->Translate,t,Ft,Gt,C,qlist[0]->x,qlist[6]->x);
		}
		pointInHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[0]->x, qlist[6]->x, qlist[0]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[0]->L, qlist[0]->E, Ft, Gt);
			createObstacle(qlist[0]->reachablePoints,points,qlist[0]->Transform,qlist[0]->Translate,t,Ft,Gt,C,qlist[0]->x,qlist[7]->x);
		}
		pointInHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[0]->reachablePoints, insideHull, qlist[0]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[0]->x, qlist[7]->x, qlist[0]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);*/
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[1]->L, qlist[1]->E, Ft, Gt);
			createObstacle(qlist[1]->reachablePoints,points,qlist[1]->Transform,qlist[1]->Translate,t,Ft,Gt,C,qlist[1]->x,qlist[4]->x);
		}
		pointInHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[1]->x, qlist[4]->x, qlist[1]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[1]->L, qlist[1]->E, Ft, Gt);
			createObstacle(qlist[1]->reachablePoints,points,qlist[1]->Transform,qlist[1]->Translate,t,Ft,Gt,C,qlist[1]->x,qlist[5]->x);
		}
		pointInHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[1]->x, qlist[5]->x, qlist[1]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[1]->L, qlist[1]->E, Ft, Gt);
			createObstacle(qlist[1]->reachablePoints,points,qlist[1]->Transform,qlist[1]->Translate,t,Ft,Gt,C,qlist[1]->x,qlist[6]->x);
		}
		pointInHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[1]->x, qlist[6]->x, qlist[1]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[1]->L, qlist[1]->E, Ft, Gt);
			createObstacle(qlist[1]->reachablePoints,points,qlist[1]->Transform,qlist[1]->Translate,t,Ft,Gt,C,qlist[1]->x,qlist[7]->x);
		}
		pointInHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[1]->reachablePoints, insideHull, qlist[1]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[1]->x, qlist[7]->x, qlist[1]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);*/
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[2]->L, qlist[2]->E, Ft, Gt);
			createObstacle(qlist[2]->reachablePoints,points,qlist[2]->Transform,qlist[2]->Translate,t,Ft,Gt,C,qlist[2]->x,qlist[4]->x);
		}
		pointInHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[2]->x, qlist[4]->x, qlist[2]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[2]->L, qlist[2]->E, Ft, Gt);
			createObstacle(qlist[2]->reachablePoints,points,qlist[2]->Transform,qlist[2]->Translate,t,Ft,Gt,C,qlist[2]->x,qlist[5]->x);
		}
		pointInHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[2]->x, qlist[5]->x, qlist[2]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[2]->L, qlist[2]->E, Ft, Gt);
			createObstacle(qlist[2]->reachablePoints,points,qlist[2]->Transform,qlist[2]->Translate,t,Ft,Gt,C,qlist[2]->x,qlist[6]->x);
		}
		pointInHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[2]->x, qlist[6]->x, qlist[2]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[2]->L, qlist[2]->E, Ft, Gt);
			createObstacle(qlist[2]->reachablePoints,points,qlist[2]->Transform,qlist[2]->Translate,t,Ft,Gt,C,qlist[2]->x,qlist[7]->x);
		}
		pointInHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[2]->reachablePoints, insideHull, qlist[2]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[2]->x, qlist[7]->x, qlist[2]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);*/
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[3]->L, qlist[3]->E, Ft, Gt);
			createObstacle(qlist[3]->reachablePoints,points,qlist[3]->Transform,qlist[3]->Translate,t,Ft,Gt,C,qlist[3]->x,qlist[4]->x);
		}
		pointInHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[3]->x, qlist[4]->x, qlist[3]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[3]->L, qlist[3]->E, Ft, Gt);
			createObstacle(qlist[3]->reachablePoints,points,qlist[3]->Transform,qlist[3]->Translate,t,Ft,Gt,C,qlist[3]->x,qlist[5]->x);
		}
		pointInHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[3]->x, qlist[5]->x, qlist[3]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[3]->L, qlist[3]->E, Ft, Gt);
			createObstacle(qlist[3]->reachablePoints,points,qlist[3]->Transform,qlist[3]->Translate,t,Ft,Gt,C,qlist[3]->x,qlist[6]->x);
		}
		pointInHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[3]->x, qlist[6]->x, qlist[3]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[3]->L, qlist[3]->E, Ft, Gt);
			createObstacle(qlist[3]->reachablePoints,points,qlist[3]->Transform,qlist[3]->Translate,t,Ft,Gt,C,qlist[3]->x,qlist[7]->x);
		}
		pointInHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[3]->reachablePoints, insideHull, qlist[3]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[3]->x, qlist[7]->x, qlist[3]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);*/
		//--------------------------------------------------------------------------------------------------------------------------
		...
		distances.clear();
		normalVectors.clear();
		qlist[4]->reachablePoints.clear();
		Ft = identity<X_DIM>();
		Gt = zeros<X_DIM,V_DIM>();
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[0]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[0]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[0]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[0]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[1]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[1]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[1]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[1]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[2]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[2]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[2]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[2]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[3]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[3]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[3]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[3]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[5]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[5]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[6]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[6]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[4]->L, qlist[4]->E, Ft, Gt);
			createObstacle(qlist[4]->reachablePoints,points,qlist[4]->Transform,qlist[4]->Translate,t,Ft,Gt,C,qlist[4]->x,qlist[7]->x);
		}
		pointInHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[4]->reachablePoints, insideHull, qlist[4]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[4]->x, qlist[7]->x, qlist[4]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);*/
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 4 Half Planes
		computeHalfPlanes(qlist[4]->x, qlist[4]->vGoal, qlist[4]->newV, distances, normalVectors, insideHull);
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		/*// Rotor 5 to 0
		distances.clear();
		normalVectors.clear();
		qlist[5]->reachablePoints.clear();
		Ft = identity<X_DIM>();
		Gt = zeros<X_DIM,V_DIM>();
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[0]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[0]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[0]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[0]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[1]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[1]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[1]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[1]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[2]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[2]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[2]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[2]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[3]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[3]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[3]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[3]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[4]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[4]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[6]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[6]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[5]->L, qlist[5]->E, Ft, Gt);
			createObstacle(qlist[5]->reachablePoints,points,qlist[5]->Transform,qlist[5]->Translate,t,Ft,Gt,C,qlist[5]->x,qlist[7]->x);
		}
		pointInHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[5]->reachablePoints, insideHull, qlist[5]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[5]->x, qlist[5]->x, qlist[5]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 5 Half Planes
		computeHalfPlanes(qlist[5]->x, qlist[5]->vGoal, qlist[5]->newV, distances, normalVectors, insideHull);
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 6 to 0
		distances.clear();
		normalVectors.clear();
		qlist[6]->reachablePoints.clear();
		Ft = identity<X_DIM>();
		Gt = zeros<X_DIM,V_DIM>();
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[0]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[0]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[0]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[0]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[1]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[1]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[1]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[1]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[2]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[2]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[2]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[2]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[3]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[3]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[3]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[3]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[4]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[4]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[5]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[5]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[6]->L, qlist[6]->E, Ft, Gt);
			createObstacle(qlist[6]->reachablePoints,points,qlist[6]->Transform,qlist[6]->Translate,t,Ft,Gt,C,qlist[6]->x,qlist[7]->x);
		}
		pointInHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[7]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[6]->reachablePoints, insideHull, qlist[6]->x, qlist[7]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[6]->x, qlist[7]->x, qlist[6]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 6 Half Planes
		computeHalfPlanes(qlist[6]->x, qlist[6]->vGoal, qlist[6]->newV, distances, normalVectors, insideHull);
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 7 to 0
		distances.clear();
		normalVectors.clear();
		qlist[7]->reachablePoints.clear();
		Ft = identity<X_DIM>();
		Gt = zeros<X_DIM,V_DIM>();
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[0]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[0]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[0]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[0]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[1]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[1]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[1]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[1]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[2]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[2]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[2]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[2]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[3]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[3]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[3]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[3]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[4]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[4]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[4]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[4]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[5]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[5]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[5]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[5]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		...
		for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
			findFG(A,B,qlist[7]->L, qlist[7]->E, Ft, Gt);
			createObstacle(qlist[7]->reachablePoints,points,qlist[7]->Transform,qlist[7]->Translate,t,Ft,Gt,C,qlist[7]->x,qlist[6]->x);
		}
		pointInHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[6]->x);
		if(insideHull) { // if the point is inside the convex hull
			convexHull(qlist[7]->reachablePoints, insideHull, qlist[7]->x, qlist[6]->x, distance, normalVector);
		}
		else // If the point is outside the convex hull, run GJK
		{
			run_gjk(qlist[7]->x, qlist[6]->x, qlist[7]->reachablePoints, distance, normalVector);
		}	
		distances.push_back(0.5*distance);
		normalVectors.push_back(normalVector);
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 7 Half Planes
		computeHalfPlanes(qlist[7]->x, qlist[7]->vGoal, qlist[7]->newV, distances, normalVectors, insideHull);*/
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 0 Update
		...
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 4 Update
		qlist[4]->vGoal = qlist[4]->newV;
		u = qlist[4]->findU();
		qlist[4]->propagateU(u);
		kalmanFilter1(qlist[4]->x, qlist[4]->Rot, u, M, qlist[4]->P);
		z = sampleGaussian(h(qlist[4]->xTrue, qlist[4]->RotTrue), N);
		kalmanFilter2(qlist[4]->x, qlist[4]->Rot, z, N, qlist[4]->P);
		qlist[4]->visualize(t*dt);
		qlist[4]->vGoal = VGoal4;
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		/*// Rotor 5 Update
		qlist[5]->vGoal = qlist[5]->newV;
		u = qlist[5]->findU();
		qlist[5]->propagateU(u);
		kalmanFilter1(qlist[5]->x, qlist[5]->Rot, u, M, qlist[5]->P);
		z = sampleGaussian(h(qlist[5]->xTrue, qlist[5]->RotTrue), N);
		kalmanFilter2(qlist[5]->x, qlist[5]->Rot, z, N, qlist[5]->P);
		qlist[5]->visualize(t*dt);
		qlist[5]->vGoal = VGoal5;
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 6 Update
		qlist[6]->vGoal = qlist[6]->newV;
		u = qlist[6]->findU();
		qlist[6]->propagateU(u);
		kalmanFilter1(qlist[6]->x, qlist[6]->Rot, u, M, qlist[6]->P);
		z = sampleGaussian(h(qlist[6]->xTrue, qlist[6]->RotTrue), N);
		kalmanFilter2(qlist[6]->x, qlist[6]->Rot, z, N, qlist[6]->P);
		qlist[6]->visualize(t*dt);
		qlist[6]->vGoal = VGoal6;
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
		// Rotor 7 Update
		qlist[7]->vGoal = qlist[7]->newV;
		u = qlist[7]->findU();
		qlist[7]->propagateU(u);
		kalmanFilter1(qlist[7]->x, qlist[7]->Rot, u, M, qlist[7]->P);
		z = sampleGaussian(h(qlist[7]->xTrue, qlist[7]->RotTrue), N);
		kalmanFilter2(qlist[7]->x, qlist[7]->Rot, z, N, qlist[7]->P);
		qlist[7]->visualize(t*dt);
		qlist[7]->vGoal = VGoal7;*/
		//--------------------------------------------------------------------------------------------------------------------------
		//--------------------------------------------------------------------------------------------------------------------------
	}
