...
#define NUM_QUADS 24			// Number of quadrotors
...
bool drawEllipses = false;
	...
	clock_t quadObstacleStart[NUM_QUADS][TIME_STEPS];
	clock_t quadObstacleStop[NUM_QUADS][TIME_STEPS];
	clock_t quadMoveStart[NUM_QUADS][TIME_STEPS];
	clock_t quadMoveStop[NUM_QUADS][TIME_STEPS];
	clock_t start, stop;
	double quadObstacleTotal[NUM_QUADS][TIME_STEPS];
	double quadMoveTotal[NUM_QUADS][TIME_STEPS];
...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	double height= 0.9;
	// Quadrotor 0
	xInit[0] = 6.0; xInit[1] = 0.0; xInit[2] = height;
	xInit[3] = 0.0; xInit[4] = 0.0; xInit[5] = 0.0;
	tempPGoal[0] = -6.0; tempPGoal[1] = 0.0; tempPGoal[2] = height;
	...
	// Quadrotor 2
	xInit[0] = 0.0; xInit[1] = 6.0; xInit[2] = height;
	xInit[3] = 0.0; xInit[4] = 0.0; xInit[5] = 0.0;
	...
	for(size_t t = 0; t < TIME_STEPS; ++t) {
		...
		for(size_t i = 0; i  < NUM_QUADS; ++i) {
			...
			quadObstacleStop[i][t] = clock();
		}
		for(size_t i = 0; i < NUM_QUADS; ++i) {
			quadMoveStart[i][t] = clock();
	...
	double obstacleTime[NUM_QUADS], moveTime[NUM_QUADS];
	for(int i = 0; i < NUM_QUADS; ++i) {
		for(int t = 0; t< TIME_STEPS; ++t) {
			quadObstacleTotal[i][t] = quadObstacleStop[i][t] - quadObstacleStart[i][t];
			quadMoveTotal[i][t] = quadMoveStop[i][t] - quadMoveStart[i][t];
			obstacleTime[i] += quadObstacleTotal[i][t];
			moveTime[i] += quadMoveTotal[i][t];
			outputFile << "Quadrotor " << i << ", Time Step " << t << std::endl;
			outputFile << quadObstacleStart[i][t] << "     " << quadObstacleStop[i][t] << "    " << quadObstacleTotal[i][t] << std::endl;
			outputFile << quadMoveStart[i][t] << "    " << quadMoveStop[i][t] << "    " << quadMoveTotal[i][t] << std::endl;
		}
		outputFile << std::endl << "-------------------------------------------------------------------------" << std::endl;
		outputFile << std::endl << "Quadrotor " << i << "total time: " << obstacleTime[i] + moveTime[i] << std::endl;
		outputFile << std::endl << "-------------------------------------------------------------------------" << std::endl;
	}
	outputFile.close();
