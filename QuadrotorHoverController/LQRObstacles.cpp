...
inline void controlMatrices(const Input& uGoal, const Matrix<X_DIM,1>& xstar, Matrix<X_DIM,X_DIM>& A, Matrix<X_DIM,U_DIM>& B, Matrix<X_DIM,1>& c, Matrix<U_DIM,X_DIM>& L, Matrix<U_DIM,V_DIM>& E, Matrix<U_DIM,1>& l, Matrix<V_DIM,X_DIM>& Lh, Matrix<V_DIM,V_DIM>& Eh) {
  ...
  for (int i = 0; i < 300; ++i) {
   ...
   Matrix<X_DIM,X_DIM> Qptilde = ~Pmap*Qp*Pmap + 0.05*~L*R*L;
   Matrix<V_DIM,V_DIM> Rtilde = 0.05*~E*R*E;//identity<V_DIM>(); 
   Matrix<V_DIM,X_DIM> Ptilde = 0.05*~E*R*L;//zeros<V_DIM, X_DIM>(); //
...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	tempPGoal[0] = -4.0; tempPGoal[1] = 0.0; tempPGoal[2] = 2.0;
	...
	xInit[3] = 0.0; xInit[4] = 0.0; xInit[5] = 0.0;
	tempPGoal[0] = 0.0; tempPGoal[1] = -4.0; tempPGoal[2] = 2.0;
	pGoal.push_back(tempPGoal);
	...
	tempPGoal[0] = -arm/2.0; tempPGoal[1] = -arm/2.0; tempPGoal[2] = 2.0;
	pGoal.push_back(tempPGoal);
	...
	for(size_t t = 0; t < TIME_STEPS; ++t) {
		...
		for(size_t i = 0; i  < NUM_QUADS; ++i) {
			...
			for(size_t j = 0; j < NUM_QUADS; ++j) {
				...
				/*qlist[i]->ellipsoids.clear();
				qlist[j]->reachablePoints.clear();
				Ft = identity<X_DIM>();
				Gt = zeros<X_DIM,V_DIM>();
				for(size_t k = 0; k < OBSTACLE_STEPS; ++k) {
					findFG(A,B, qlist[i]->L, qlist[i]->E, Ft, Gt);
					createObstacle(qlist[i]->ellipsoids, floorPoints, qlist[i]->Transform, qlist[i]->Translate, t, Ft, Gt, C, qlist[i]->x, floorState,1);
				}
				pointInHull(qlist[i]->ellipsoids, insideHull, qlist[i]->x, floorState);
				if(insideHull)
					convexHull(qlist[i]->ellipsoids, insideHull, qlist[i]->x, floorState, distance, normalVector);
				else
					run_gjk(qlist[i]->x, floorState, qlist[i]->ellipsoids, distance, normalVector);
				createHalfPlanes(qlist[i]->x, distance, normalVector, insideHull, orcaPlanes_);*/
