...
#define OBSTACLE_STEPS 30	// Time to check for obstacle
#define NUM_POINTS 100		// Number of points to make up the ellipse
#define XYRADIUS 0.26
#define ZRADIUS 0.75 
bool drawEllipses = false;

clock_t obstacleCreationStart[NUM_QUADS][NUM_QUADS][TIME_STEPS], obstacleCreationStop[NUM_QUADS][NUM_QUADS][TIME_STEPS];
double obstacleCreationTotal[NUM_QUADS][NUM_QUADS][TIME_STEPS];
clock_t halfPlaneStart[NUM_QUADS][NUM_QUADS][TIME_STEPS], halfPlaneStop[NUM_QUADS][NUM_QUADS][TIME_STEPS];
double halfPlaneTotal[NUM_QUADS][NUM_QUADS][TIME_STEPS];
clock_t newVStart[NUM_QUADS][TIME_STEPS], newVStop[NUM_QUADS][TIME_STEPS];
double newVTotal[NUM_QUADS][TIME_STEPS];
clock_t fileStart, fileStop;
double file1Total, file2Total;
...
int cal_floor;
int cal_ellipse;
std::vector<int> quadrotors[NUM_QUADS][TIME_STEPS];
...
inline void controlMatrices(const Input& uGoal, const Matrix<X_DIM,1>& xstar, Matrix<X_DIM,X_DIM>& A, Matrix<X_DIM,U_DIM>& B, Matrix<X_DIM,1>& c, Matrix<U_DIM,X_DIM>& L, Matrix<U_DIM,V_DIM>& E, Matrix<U_DIM,1>& l, Matrix<V_DIM,X_DIM>& Lh, Matrix<V_DIM,V_DIM>& Eh) {
  ...
  for (int i = 0; i < 300; ++i) {
   ...
   Matrix<V_DIM,V_DIM> Rtilde = 0.25*~E*R*E;//identity<V_DIM>(); 
   Matrix<V_DIM,X_DIM> Ptilde = 0.25*~E*R*L;//zeros<V_DIM, X_DIM>(); //
   Matrix<X_DIM,X_DIM> Atilde = A+B*L;
   Matrix<X_DIM,V_DIM> Btilde = B*E;

     // Solve riccati equation to find S (naive implementation)
  // Matrix<X_DIM,X_DIM> S = Q;
  Matrix<X_DIM, X_DIM> Stilde = Qptilde; // + Qx;
...
inline void createFloorSpheres(std::vector< Matrix<3,1> >& points) {
	/*double dlong = M_PI*(3.0-sqrt(5.0));
	double dz = 2.0/NUM_POINTS;
	double longt = 0;
	double z = 1.0 -dz/2.0;
	...
	for(int i = 0; i<NUM_POINTS; i++) {
		tempPoint[0] = XYRADIUS*cos(longt)*sqrt(1-z*z);
		tempPoint[1] = XYRADIUS*sin(longt)*sqrt(1-z*z);
		tempPoint[2] = ZRADIUS*z;
		points.push_back(tempPoint);
		z -= dz;
		longt += dlong;
	}*/
	Matrix<3,1> tempPoint;
	for(int zcount = 0; zcount < 20; zcount++) {
		for(int ycount = 0; ycount < 80; ycount++) {
			for(int xcount = 0; xcount < 80; xcount++) {
				tempPoint[0] = -10.0+xcount/4.0;
				tempPoint[1] = -10.0+ycount/4.0;
				tempPoint[2] = -ZRADIUS + ZRADIUS*xcount/10.0;
				points.push_back(tempPoint);
			}
		}
	}
...
void convexHull(const std::vector< Matrix<3,1>>& reachablePoints, bool& insideHull, const State& x0, const State& x1, double& distance, Matrix<3,1>& normal) {
	...
	outputFile.close();
	fileStop = clock();
	file1Total = (double) (fileStop - fileStart)/CLOCKS_PER_SEC;
	...
	fileStop = clock();
	file2Total = (double) (fileStop - fileStart)/CLOCKS_PER_SEC;
	/*while(inputFile.good()) {
		if(count == 0) {
			getline(inputFile, dump);
			count++;
...
bool linearProgram1(const std::vector<Plane>& planes, size_t planeNo, Line& line, float radius, const Vector3& optVelocity, bool directionOpt, Vector3& result)
  ...
  /*void computeHalfPlanes(const State& x0, const Matrix<3,1>& goalVel, Matrix<3,1>& newVel, const std::vector<double>& distance, const std::vector<Matrix<3,1>>& normals){
...
int _tmain(int argc, _TCHAR* argv[]) {
	...
	double height=2.5;
	// Quadrotor 0
	xInit[0] = -4.0; xInit[1] = 0.0; xInit[2] = height;
	xInit[3] = 0.0; xInit[4] = 0.0; xInit[5] = 0.5;
	tempPGoal[0] = 6.0; tempPGoal[1] = 0.0; tempPGoal[2] = height+0.5;
	...
	std::vector<Matrix<3,1>> floorPoints;
	createFloorSpheres(floorPoints);
	State floorState = zeros<X_DIM,1>();
	...
	for(size_t t = 0; t < TIME_STEPS; ++t) {
		...
		for(size_t i = 0; i  < NUM_QUADS; ++i) {
			...
			for(size_t j = 0; j < NUM_QUADS; ++j) {
				//obstacleCreationStart[i][j][t] = clock();
				...
				if(i==j);
					...
					numObstacle= qlist[i]->reachablePoints.size();
					//halfPlaneStart[i][j][t] = clock();
					if(numObstacle > 4) {	
					    pointInHull(qlist[i]->reachablePoints, insideHull, qlist[i]->x, qlist[j]->x);
					    if(insideHull){
							file1Total = 0; file2Total = 0;
					    ...
					    distance *=0.5;
					    createHalfPlanes(qlist[i]->x, distance, normalVector, insideHull, orcaPlanes_);
					}
					//halfPlaneStop[i][j][t] = clock();
					//halfPlaneTotal[i][j][t] = (double)(halfPlaneStop[i][j][t]-halfPlaneStart[i][j][t])/CLOCKS_PER_SEC;// -(file1Total + file2Total);
				}
				//obstacleCreationStop[i][j][t] = clock();
				//obstacleCreationTotal[i][j][t] = (double)(obstacleCreationStop[i][j][t] - obstacleCreationStart[i][j][t])/CLOCKS_PER_SEC - halfPlaneTotal[i][j][t];
			}
			//newVStart[i][t] = clock();
			calculateNewV(orcaPlanes_, qlist[i]->vGoal, qlist[i]->newV);
			//newVStop[i][t] = clock();
			//newVTotal[i][t] = (double)(newVStop[i][t] - newVStart[i][t])/CLOCKS_PER_SEC;
		}
		timeCount++;
		for(size_t i = 0; i < NUM_QUADS; ++i) {
			int k = t;
			...
			if(timeCount==4) {
				CAL_CloneGroup(&quadrotors, qlist[i]->cal_quadrotor, 0, false);
				CAL_CloneGroup(&ellipses, qlist[i]->cal_ellipse, 0, false);
				float p[3] = {(float) qlist[i]->xTrue[0], (float) qlist[i]->xTrue[1], (float) qlist[i]->xTrue[2]};
				Matrix<4> q = quatFromRot(qlist[i]->RotTrue);
				float o[4] = {(float) q[0], (float) q[1], (float) q[2], (float) q[3]};
				CAL_SetGroupPosition(quadrotors, p[0], p[1], p[2]);
				CAL_SetGroupQuaternion(quadrotors, q[0],q[1],q[2],q[3]);
				CAL_SetGroupPosition(ellipses,p[0],p[1],p[2]);
			}
			qlist[i]->vGoal = qlist[i]->findVGoal();

			/*if(i==0){
				CAL_scalar eye[3], view[3], up[3];
				up[0]=0; up[1] = 0; up[2] =1;
				eye[0] = qlist[i]->x[0] - qlist[i]->x[3]; eye[1] = qlist[i]->x[1] - qlist[i]->x[4]; eye[2] = qlist[i]->x[2]+1.25;
				view[0] = qlist[i+1]->x[0]; view[1] = qlist[i+1]->x[1]; view[2] = qlist[i+1]->x[2]-0.5;
			CAL_AddViewKeyState(0,(float) t*dt, eye, view, up);
			}*/
		}
		if(timeCount==4)
			timeCount=0;
	...
	/*std::ofstream outputFile;
	outputFile.open("obstacleCreation.txt");
	for(int t = 0; t < TIME_STEPS; ++t) {
		for(int i = 0; i < NUM_QUADS; ++i) {
			for(int j = 0; j < NUM_QUADS; ++j) {
				outputFile << "Time Step: " << t << std::endl;
				outputFile << "Quadrotor " << i << " to " << j << ": " << obstacleCreationTotal[i][j][t] << std::endl;
			}
		}
	}
	outputFile.close();
	outputFile.open("halfPlaneCreation.txt");
	for(int t = 0; t < TIME_STEPS; ++t) {
		for(int i = 0; i < NUM_QUADS; ++i) {
			for(int j = 0; j < NUM_QUADS; j++) {
				outputFile << "Time Step: " << t << std::endl;
				outputFile << "Quadrotor " << i << " to " << j << ": " << halfPlaneTotal[i][j][t] << std::endl;
			}
		}
	}
	outputFile.close();
	outputFile.open("newVCalc.txt");
	for(int t = 0; t < TIME_STEPS; ++t) {
		for(int i = 0; i < NUM_QUADS; ++i) {
			outputFile << "Time Step: " << t << std::endl;
			outputFile << "Quadrotor " << i <<": " << newVTotal[i][t] << std::endl;
		}
	}
	outputFile.close();*/
	int k;
